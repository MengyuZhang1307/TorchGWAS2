...
void fill_mat(SpaMat &J, const int numRows, const int numCols, std::ext::V_int& indixes_col, int value)
{
...
void fill_J(std::ext::V_string id_include, SpaMat &J)
{
    std::ext::V_string unique_id_include = unique_id(id_include);
    std::ext::V_int indixes_col = match_indices(id_include, unique_id_include); 
    std::ext::V_int indixes_col_unique = unique_id(indixes_col);
    J.resize(id_include.size(), indixes_col_unique.size());
    fill_mat(J, id_include.size(), indixes_col_unique.size(), indixes_col, 1);
    J = J.transpose();
}
...
//Create matrix X covariate data by adding extra col fill with one and cov data
...
void GMMAT::calc_rand_effect(int &kins_size, int ng)
{
    ...
    {
        SparseInverse spmi_cov;
        SparseInverse spmi_slope;
        SpaMat spm = m_vkins_sp[i].get_spmat();
        SpaMat spm_left = spm * m_rand_slope.asDiagonal();
        SpaMat spm_right = spm_left.transpose();
        SpaMat spm_cov = spm_left + spm_right;
        spm_cov.makeCompressed();
        spmi_cov.set_spmat(spm_cov);
        m_vkins_sp[kins_size + i] = spmi_cov;
        SpaMat spm_slope = spm_right * m_rand_slope.asDiagonal();
        spm_slope.makeCompressed();
        spmi_slope.set_spmat(spm_slope);
        m_vkins_sp[kins_size * 2 + i] = spmi_slope;
                            ...
                            double tau_max, int tau_region)
{
    ...
    glmmkin_results.c2 = glmmkin.c2;
    return glmmkin_results;    
}